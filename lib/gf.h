...
namespace detail {
    ...
    // Exclude both the most significant monomial (since we know it's x^k) 
    // and the least significant (x^0)
    ...
    T reduce_once(const T& x, const std::tuple<int, int, int>& red) { // Pentanomial
        T hi = x >> k;
        return (x & make_mask<T, k>()) ^ hi ^ (hi << std::get<0>(red)) 
                              ^ (hi << std::get<1>(red)) ^ (hi << std::get<2>(red));
    }
